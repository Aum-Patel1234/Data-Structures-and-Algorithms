...
public class BankingManagementSystems {
    public static void main(String[] args) {
        ...
        try {
            ...
            System.out.println("Error: No input available. Please provide valid input.");
        } finally {
            sc.close(); // Close the scanner in the finally block to ensure resources are properly
                        // released
        }
...
class Banking extends User {
    Admin admin;
    public List<User> users; // [user1 , user2 ]
        ...
        do {
            ...
            switch (choice) {
                ...
                case 3:
                    ...
                    break;
                case 4:
                    System.out.println("\nEnter name of the Projects.User:");
                    String name = scanner.next();
                    System.out.println("\nEnter Password of the Projects.User:");
    ...
    private void PreviousTransaction() {
        User user;
        Scanner sc = new Scanner(System.in);
        System.out.println("Enter Your Projects.User Id :");
        int user_ID = sc.nextInt();
        user = getId(user_ID);
        if (user != null) {
            user.getPrev_Transaction();
        } else {
            System.out.println("Your Account was not found , Pls Enter Correct/Valid Projects.User id.");
        }
    ...
    private void deposit() {
        ...
        user = getId(user_ID);
        if (user != null && password_Verification(user)) {
            System.out.println("\nYour Projects.User Name : " + user.name);
            System.out.println("Your Projects.User ID : " + user.User_id + "\n");
            ...
            System.out.println("Projects.User Id not Found . Pls Try again .");
        }
        ...
        if (user != null && password_Verification(user)) {
            ...
            if (c == 'y' || c == 'Y') {
                while (true) {
                    ...
                    user2 = getId(id2);
                    while (user2 == null) {
                        System.out.println("\nAccount id not found !! Pls write valid Projects.User Id");
                        id2 = sc.nextInt();
                        user2 = getId(id2);
                    }
    ...
    private void signUp() {
        ...
        GenerateUserID(user);
        System.out.println("Your Projects.User name is : " + user.name + "\nYour Projects.User Id is :" + user.User_id);
        System.out.println(
    ...
    private void GenerateUserID(User user) {
        ...
        user.User_id = id;
        // for (int i = 0; i < users.toArray().length; i++) { was trying to not repeat
        // two ids but in this way even if i increase id by 1 there is no
        // int temp = users.get(i).User_id; guarantee that the ids assigned before may
        // not be same after incrementing i
        // if(id == temp){
        // id++;
        // }
        // }
...
class Admin {
    ...
    protected static void addUser(List<User> users, User user) {
        users.add(user);
        System.out.println("Projects.User " + user.name + " added successfully!");
    }
