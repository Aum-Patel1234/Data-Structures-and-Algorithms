...
bool Graph::isBipartiteDFSHelper(const std::vector<std::vector<int>>& graph, std::vector<int>& color, int curr,
                                 int currentColor) {
  color[curr] = currentColor;

  for (auto& neighbor : graph[curr]) {
    if (color[neighbor] == -1) {
      if (!isBipartiteDFSHelper(graph, color, neighbor, !currentColor)) return false;
...
bool Graph::isBipartiteDFS(std::vector<std::vector<int>>& graph) {
  ...
  for (int i = 0; i < V; i++) {
    if (color[i] == -1 && !isBipartiteDFSHelper(graph, color, i, 0)) return false;
  }
...
int main() {
  ...
  Graph g;
  std::cout << "The graph even edges is Bipartite (DFS) - " << g.isBipartiteDFS(graph) << std::endl;
  std::cout << "The graph is Bipartite (DFS) - " << g.isBipartiteDFS(graph1) << std::endl << std::endl;
  std::cout << "The graph even edges is Bipartite (BFS) - " << g.isBipartiteBFS(graph) << std::endl;
  std::cout << "The graph is Bipartite (BFS) - " << g.isBipartiteBFS(graph1) << std::endl;
