...
// Time  : O(m*n)
// Space : O(m+n)
unsigned long paths_dp(std::unordered_map<std::pair<int, int>, unsigned long, pair_hash>& memo, int m, int n) {
  if (m == 1 && n == 1) return 1;
  if (m == 0 || n == 0) return 0;
  if (memo.count({m, n})) return memo[{m, n}];

  return memo[{m, n}] = paths_dp(memo, m - 1, n) + paths_dp(memo, m, n - 1);
}
unsigned long uniquePaths(int m, int n) {
  std::unordered_map<std::pair<int, int>, unsigned long, pair_hash> memo;
  return paths_dp(memo, m, n);
}
